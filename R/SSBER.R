...
#'
#' @return iSSBER returns a Seurat object that contains the the batch-corrected expression matrix for batches
#'
...
#' @references Yuchen Yang, Gang Li, Li Qian, Yun Li. iSSBER 2020
#'
#' @examples
#' # Load the example data data_SSBER
#' data("data_iSSBER")
#'
...
#'
#' library(SSBER)
#' batch.cluster.labels <- unifiedClusterLabelling(data_SSBER$batch1.mat, data_iSSBER$batch2.mat, features.use = markers,
#'                                                 cluster.labels = cluster.info, min.perc = 0.3)
#' names(batch.cluster.labels[[1]]) <- colnames(data_iSSBER$batch1.mat)
#' names(batch.cluster.labels[[2]]) <- colnames(data_iSSBER$batch2.mat)
#'
#' # Construct the input object for batches using Seurat
#' library(Seurat)
#' merge <- CreateSeuratObject(counts = cbind(data_iSSBER$batch1.mat, data_iSSBER$batch2.mat), min.cells = 0, min.features = 0)
#' batch_id <- c(rep("batch1", ncol(data_iSSBER$batch1.mat)), rep("batch2", ncol(data_iSSBER$batch2.mat)))
...
#' corrected.results <- iSSBER(object.list = merge.list, batch.cluster.labels = batch.cluster.labels,
...
iSSBER <- function(
  ...
  object.anchors_1round <- iSSBER_FindSSBERs(object.list = object.list, batch.cluster.labels = batch.cluster.labels, matched.clusters = matched.clusters, assay = assay,
  ...
  if (strategy == "Short.run"){
    ...
    for(k in 2:iterations){
      ...
      object.anchors_after_correction <- iSSBER_FindSSBERs_aftercorrection(object.list = corrected.object, assay = rep("integrated", length(merge.list)),
    ...
    for(k in 2:step){
      ...
      object.anchors_after_correction <- iSSBER_FindIntegrationAnchors_aftercorrection(object.list = corrected.object, assay = rep("integrated", length(merge.list)),
                                                                                      batch.cluster.labels = batch.cluster.labels, matched.clusters = matched.clusters, reference = NULL,
                                                                                      anchor.features = anchor.features, scale = scale, normalization.method = "LogNormalize",
                                                                                      reduction = reduction, l2.norm = l2.norm, dims = dims, k.anchor = k.anchor,
                                                                                      k.filter = k.filter, k.score = k.score, max.features = max.features,
